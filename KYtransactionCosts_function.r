...
KYtransactionCosts <- function(donosi,zacPort,sellCosts = 4, buyCosts = 5, 
								upperBound = NULL, lowerBound = NULL, 
								od=5*10^-4, do=0.002, po =1*10^-4 ){
  ## izracuna portfelj ce imamo transakcijske stroske sellCosts in buyCosts in zacetni portfelj zacPort
  ...
  ## omejitve y,z,x
  omejitve <- matrix(0, dimObdobji+2+2*dimPortfelj+1,2*dimObdobji+ dimPortfelj+2*dimPortfelj)
  ...
  ## stroski nakupa 
  omejitve[(dimObdobji+2),(2*dimObdobji+ dimPortfelj+1):ncol(omejitve)] <- -c(rep(buyCosts, dimPortfelj), rep(sellCosts, dimPortfelj))
  ...
  omejitve[(dimObdobji+2+2*dimPortfelj+1),(2*dimObdobji+ dimPortfelj+1):(2*dimObdobji+ dimPortfelj+2*dimPortfelj)] <- c(rep(1, dimPortfelj), rep(-1, dimPortfelj))
  ## predznaki
  ...
  for(i in seq(od,do, by = po)){
    ...
    minR <- i
    #minR <- 0.0005
    b0 <- c(matrix(0, dimObdobji,1), c(1,minR), zacPort, -zacPort,0)
    ...
    prodajaNakup <- (2*nrow(donosi)+ncol(donosi)+1):length(y)
    cena <- c(rep(buyCosts, dimPortfelj), rep(sellCosts, dimPortfelj))
    stroski <- y[prodajaNakup]%*%cena
    ret1 <- cbind(ret1,y[portfelj]%*% Edonosi-stroski)
